...
| ⛰️ **Suspension demo** — drive the rough-terrain test lane and watch the full rocker-bogie (incl. differential) articulate | `ros2 launch rover_gazebo sim.launch.py world:=mars_rough_world.sdf` + teleop, drive along **+X** |
...
|:---:|:---:|:---:|:---:|
| 10×10 m walled room, 4 obstacles — ideal for first mapping runs | 22×22 m Mars terrain — 34 boulders, crater, canyon, ramps, rocky ridge | 20×20 m rough-terrain suspension test lane — bump bars, staggered bars, log climbs, pebble field (teleop) | 22×22 m **grand demo** — rocks + crater + ridge for SLAM/Nav2 **plus** 260 climbable pebbles and logs, so the bogies articulate while the rover navigates itself |
...
- **Suspension in simulation** — the rocker-bogie differential is a closed kinematic loop, which URDF cannot express directly. Three modes: (1) flat SLAM worlds rigidify the suspension; (2) `mars_rough_world.sdf` runs the **full** suspension — passive rocker/bogie revolutes with the differential bar emulated by a URDF `<mimic>` constraint (right rocker = −left rocker) enforced by the bullet-featherstone engine — the chassis stays within ~7° of level while bogies swing ±20°, and climbing a 4 cm log lifts the chassis only ~1.5 cm (wheel torque capped at a realistic 0.65 N·m stall so it rock-crawls instead of wheelie-flipping); (3) `mars_demo_world.sdf` frees just the **bogies** (independent joints, no closed loop) under the default DART engine, keeping the calibrated skid-steer odometry — so the suspension visibly works *while Nav2 drives autonomously*. Bullet can't do controllable skid-steer rotation (wide wheel contact grips torsionally), which is why the full-differential mode stays a teleop demo.
