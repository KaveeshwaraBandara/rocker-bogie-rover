...
| `obstacle_world.sdf` *(default)* | `mars_world.sdf` |
|:---:|:---:|
| 10×10 m walled room, 4 obstacles — ideal for first mapping runs | 22×22 m Mars terrain — 34 boulders, crater, canyon, ramps, rocky ridge |
...
- **Suspension in simulation** — the rocker-bogie differential is a closed kinematic loop, which URDF cannot express; in Gazebo the suspension is rigidified (`use_sim:=true` xacro arg) while remaining fully articulated for visualization. On flat-to-moderate terrain this has no effect on SLAM or navigation.
